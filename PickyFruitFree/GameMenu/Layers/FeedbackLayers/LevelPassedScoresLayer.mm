...
addImprovedScoreSprite:(bool)addImprovedScoreSprite
{
    ...
    {   
        ...
        if (bonusScore > kInt0) 
        {
            ...
            _prevLabelBonusScoreValue = bonusScore;
            NSString *str = [[NSString alloc] initWithFormat:kStringFormatInt, _prevLabelBonusScoreValue];
            _labelBonusAnimated = [[CCLabelBMFont alloc] initWithString:str fntFile:kBmpFontLevelPassedScore32];
            [str release];
            str = nil;
        ...
        // make sure our increment is not zero
        if(bonusScore > kInt0 && _bonusScoreIncrement == kInt0)
        {
            _bonusScoreIncrement = kInt1;
        }
...
-(void)addImprovedScoreSprite
{
    if(_addImprovedScoreSprite)
    {
        ...
        [[SimpleAudioEngine sharedEngine] playEffect:kSoundEfxImprovedScore];
    }
}
...
-(void)animateBonus:(ccTime)dt
{
    if(_bonusScore > kInt0)
    {
        ...
        if(_elapsed > (_delay+kFloat0Point75)// show Bonus for about half second before animating it
        ...
        {       
            ...
            if(_bonusScoreSubStepCounter >= 1)
            {
                ...
                CCLOG(@"labelBonusValue: %d", labelBonusValue);
                if(labelBonusValue != _prevLabelBonusScoreValue)
                {
                    if(_elapsedForCounterSound > 0.05f)
                    {
                        [[SimpleAudioEngine sharedEngine] playEffect:kSoundEfxCounter];
...
- (void)update:(ccTime)dt 
{
    ...
    if(_elapsed > _delay)
    {
        ...
        {
            [self animateBonus:dt];
        }
        
        if(_scoreCounter > _prevLabelScoreValue)
        {
            if(_elapsedForCounterSound > kFloat0Point1)
            {
                [[SimpleAudioEngine sharedEngine] playEffect:kSoundEfxCounter];
                _elapsedForCounterSound = kFloat0;
            }
