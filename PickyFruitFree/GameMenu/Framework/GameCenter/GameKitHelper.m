...
//  GameMenu
//
//  Created by Damiano Fusco on 1/27/12.
...
-(id) init
{
	if ((self = [super init]))
	{
		...
		CCLOG(@"GameCenter available = %@", isGameCenterAvailable ? @"YES" : @"NO");
...
// damiano
-(void)leaderboardsReport:(int)totScore
                 totStars:(int)totStars
{
    // report score to game kit
    [self submitScore:totScore
             category:kLeaderBoardPickyFruitScore];
    [self submitScore:totStars
             category:kLeaderBoardPickyFruitStars];
}

-(void)achieveReportTimesPlayed:(int)totTimesPlayed
{
    // Achieve: Played 100 Times (kAchievePickyFruitPlayed100Times)
    if(totTimesPlayed > 0 && totTimesPlayed <= 100)
    {
        // report achievements to game kit
        GKAchievement* achievement = [[GameKitHelper sharedInstance] getAchievementByID:kAchievePickyFruitPlayed100Times];
        float percent = totTimesPlayed;
        if (percent > 0 && achievement.completed == false)
        {
            [self reportAchievementWithID:kAchievePickyFruitPlayed100Times
                          percentComplete:percent];
        }
    }
    
    // Achieve: Played 200 Times (kAchievePickyFruitPlayed200Times)
    if(totTimesPlayed > 0 && totTimesPlayed <= 200)
    {
        // report achievements to game kit
        GKAchievement* achievement = [[GameKitHelper sharedInstance] getAchievementByID:kAchievePickyFruitPlayed200Times];
        float percent = (totTimesPlayed/2.0f);
        if (percent > 0 && achievement.completed == false)
        {
            [self reportAchievementWithID:kAchievePickyFruitPlayed200Times
                          percentComplete:percent];
        }
    }
    
    /*if(self.currentGameLevel.number == 4)
    {
        // report achievements to game kit
        GKAchievement* achievement = [[GameKitHelper sharedInstance] getAchievementByID:kAchievePickTheFruit1Kiwi];
        if (achievement.completed == false)
        {
            [[GameKitHelper sharedInstance] reportAchievementWithID:kAchievePickTheFruit1Kiwi
                                                    percentComplete:100.0f];
        }
    }*/
}

-(void)achieveReportPickXYFruit:(NSString*)achieveId
                          value:(int)value
                       maxValue:(float)maxValue
{
    // Achieve: Report xy fruits picked
    GKAchievement* achievement = [[GameKitHelper sharedInstance] getAchievementByID:achieveId];
    float percent = value / maxValue * 100.0f;
    if (percent > 0 && achievement.completed == false)
    {
        [self reportAchievementWithID:achieveId
                      percentComplete:percent];
    }
}

-(void)achieveReportLevelPassed:(NSString*)achieveId
                          value:(int)value
{
    // Achieve: Report xy fruits picked
    GKAchievement* achievement = [[GameKitHelper sharedInstance] getAchievementByID:achieveId];
    if (value == kInt1 && achievement.completed == false)
    {
        [self reportAchievementWithID:achieveId
                      percentComplete:100.0f];
    }
