    ...
    if((self = [super init]))
    {
        ...
        for (TreeInfo *treeInfo in _info.treesInfo) 
        {
            ...
            for (ActorInfo *fi in treeInfo.actorsInfo) 
            {
                ...
                NSString* key = [[NSString alloc] initWithFormat:@"%d_%@", treeInfo.tag, fi.frameName];
        ...
        for (LevelGoal *goal in sharedGameManager.currentGameLevel.levelGoals)
        {
            ...
            for (TreeInfo *treeInfo in _info.treesInfo) 
            {
                for (ActorInfo *fi in treeInfo.actorsInfo) 
                {
                    if ([goal.fruitName isEqualToString:fi.frameName]) {
                        ...
                        CCLOG(@"TreeSystem: found goal fruit in tree info");
                        break;
                    }
                }
                if(found)
            ...
            {
                CCLOG(@"TreeSystem: spread the target number of fruits");
